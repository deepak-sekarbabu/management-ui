...
const DoctorAvailability = ({
    ...
    const validateRow = useCallback((index, rowData) => {
        ...
        if (rowData.consultationTime === null || rowData.consultationTime === undefined || String(rowData.consultationTime).trim() === '') {
            newErrors[index].consultationTime = 'Consultation time is required.';
        } else if (Number.isNaN(Number(rowData.consultationTime)) || Number(rowData.consultationTime) <= 0) {
            newErrors[index].consultationTime = 'Must be a positive number (minutes).';
        } else if (Number(rowData.consultationTime) > 120) { // Example: Max 2 hours
    ...
    const handleAvailabilityChange = useCallback((index, field, value) => {
    ...
    }, [editedAvailability, onAvailabilityChange, validateRow]); // Dependencies for useCallback
    ...
    const renderSelect = (index, item, field, options, label) => (
        <FormControl error={!!errors[index]?.[field]} fullWidth sx={{ minWidth: 120 }}> {/* Ensure FormControl takes full width of cell */}
            <Select
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            ...
            <TableContainer component={Paper} sx={{ mt: 2, overflowX: 'auto' }}>
                <Table sx={{ minWidth: 800 }}> {/* minWidth helps with horizontal scrolling if needed */}
                    <TableHead>
                        <TableRow>
                            {/* Column Headers */}
                            ...
                            <TableCell sx={{ fontWeight: 'bold' }}>Consultation Time (min)</TableCell>
                            <TableCell sx={{ fontWeight: 'bold' }}>Config Type</TableCell>
                            {isEditing && <TableCell align="center" sx={{ fontWeight: 'bold' }}>Actions</TableCell>}
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {editedAvailability.map((item, index) => (
                            ...
                            <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                {isEditing ? (
                                    ...
                                    <>
                                        <TableCell sx={{ verticalAlign: 'top' }}> {/* Align top for better layout with helper texts */}
                                            {renderSelect(index, item, 'availableDays', weekDays, 'Day')}
                                        </TableCell>
                                        <TableCell sx={{ verticalAlign: 'top' }}>
                                            {renderSelect(index, item, 'shiftTime', shiftTimes, 'Shift')}
                                        </TableCell>
                                        <TableCell sx={{ minWidth: { xs: 150, sm: 180 }, verticalAlign: 'top' }}> {/* Responsive minWidth */}
                                            <TimePicker
                                                value={item.shiftStartTime ? dayjs(item.shiftStartTime, 'HH:mm:ss') : null}
                                                onChange={(value) =>
                                                    handleAvailabilityChange(index, 'shiftStartTime', value ? value.format('HH:mm:ss') : '')
                                                }
                                                slotProps={{
                                                    textField: {
                                        ...
                                        <TableCell sx={{ minWidth: { xs: 150, sm: 180 }, verticalAlign: 'top' }}>
                                            <TimePicker
                                                value={item.shiftEndTime ? dayjs(item.shiftEndTime, 'HH:mm:ss') : null}
                                                onChange={(value) =>
                                                    handleAvailabilityChange(index, 'shiftEndTime', value ? value.format('HH:mm:ss') : '')
                                                }
                                                slotProps={{
                                                    textField: {
                                                        fullWidth: true,
                                        ...
                                        <TableCell sx={{ minWidth: { xs: 120, sm: 150 }, verticalAlign: 'top' }}>
                                            <TextField
                                                value={item.consultationTime === undefined ? '' : item.consultationTime} // Handle undefined for controlled input
                                        ...
                                        <TableCell sx={{ verticalAlign: 'top' }}>
                                            {renderSelect(index, item, 'configType', configTypes, 'Config Type')}
                                        </TableCell>
                                        <TableCell align="center" sx={{ verticalAlign: 'middle' }}> {/* Align button middle */}
                                            <IconButton
                ...
                {isEditing && (
                    ...
                    <Box sx={{ p: 2, display: 'flex', justifyContent: { xs: 'center', sm: 'flex-start' } }}> {/* Responsive alignment */}
                        <Button
                            variant="outlined"
